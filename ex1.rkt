...
---------------------------------------------------------------------------------------------------------------------------------
Task 2.2:
In task 2.2 we had to implement the Add operation but this too should be a variant of thedata type so I built a constructor that
accepts parameters (key, value, table).

Input: a symbol (key), a string (value), and an existing table
Output: extended table
Problems I encountered: At first I only created an empty constructor, I created a function called Add and after reading it I 
                        realized that I needed to create a constructor that accepts parameters - and so I did.
...
#|
Task 2.4:
In task 2.4 we had to implement the remove item remve-item operation: the operation should take as input a table and a symbol (key) and return a new table containing the
itemsof the original table except the item to be deleted without the key value
...
OR
If the original table was empty, we were asked to return an empty table.(test 1)
